...
const PaymentScreen = ({ route, navigation }) => {
  ...
  const pickImage = async () => {
    try {
      ...
      if (status !== 'granted') {
        window.alert('权限拒绝: 需要访问相册权限以选择图片');
        return;
      }
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const publicUrl = await uploadImage2();
      ...
      setUploaded(true);
      setUploading(false);
      window.alert('成功: 付款证明已上传');
    } catch (error) {
      console.error('Error uploading image:', error.message || error);
      window.alert('错误: 上传图片失败');
      setUploading(false);
    }
  };
  ...
  const handleUpload = () => {
    ...
    //   ]
    // );
    const isConfirmed = window.confirm('确定要上传此图片吗？');

    if (isConfirmed) {
      uploadImage(); // Call the upload function if confirmed
    }
  };
