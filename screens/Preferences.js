...
const Preferences = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        window.alert('錯誤: 無法加載偏好數據，請稍後重試');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDeletePreference = (preferenceId, categoryName) => {
    ...
    const confirmDeletePreference = async () => {
      const isConfirmed = window.confirm(`確定要刪除偏好 "${categoryName}" 嗎？`);
