...
const ProductDetail = ({ route, navigation }) => {
    ...
    try {
      ...
      if (existingItem) {
        ...
        if (newQuantity > product.stock) {
          window.alert(`錯誤: 購物車中已有 ${existingItem.quantity} 件，無法添加更多`);
          return;
        }
      }
      ...
      window.alert('成功: 商品已加入購物車');
      setModalVisible(false);
    } catch (error) {
      console.error('Error adding to cart:', error.message || error);
      window.alert(`錯誤: 操作失敗: ${error.message || '請稍後重試'}`);
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.safeArea,{maxHeight:Dimensions.get("window").height*0.87}]}>       
     <Header searchQuery={''} setSearchQuery={() => { }} />

      {/* <View style={{ flex: 1, backgroundColor: "red", height: Dimensions.get("window").height, width: "30", flexDirection: "cloumn" }}> */}
      ...
      <View style={{
        flex: 1, position: "absolute", top: windowHeight - 106
      }}>
