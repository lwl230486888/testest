...
import { View, Text, FlatList, TextInput, TouchableOpacity, StyleSheet, Alert, Image, Dimensions, ScrollView } from 'react-native';
...
const CheckoutConfirmation = ({ navigation, route }) => {
  ...
  const handleNextStep = () => {
    if (step === 1) {
      if (cartItems.length === 0) {
        window.alert('错误: 购物车为空，无法继续');
        return;
      }
      setStep(2);
    } else if (step === 2) {
      if (!address.line1 || !address.district || !address.street) {
        window.alert('错误: 请填写所有必填地址字段');
        return;
      }
      setStep(3);
    } else if (step === 3) {
      if (!paymentMethod) {
        window.alert('错误: 请选择付款方式');
        return;
      }
      ...
      if (result) {
        navigation.navigate('OrderCreation', { cartItems, total, address, paymentMethod })
      }
    }
  };
      ...
      case 1: // Review Items
        return (
          <View style={styles.stepContainer}>
            ...
            <View style={{ maxHeight: Dimensions.get("window").height<800?Dimensions.get("window").height*0.6:Dimensions.get("window").height*0.7 }}>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{
        flex: 1, position: "absolute", top: windowHeight - 170
      }}>
        <View style={[styles.buttonContainer, { width: Dimensions.get("window").width < 500 ? Dimensions.get("window").width * 0.45 : Dimensions.get("window").width * 0.8 }]}>
          {step > 1 && (
            <TouchableOpacity style={styles.backButton} onPress={() => setStep(step - 1)}>
              <Text style={styles.buttonText}>上一步</Text>
            </TouchableOpacity>
