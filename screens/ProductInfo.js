...
const ProductInfo = ({ navigation, route }) => {
  ...
  const pickImage = async () => {
    try {
      ...
      if (status !== 'granted') {
        window.alert('權限拒絕: 需要訪問相冊權限以選擇圖片');
        return;
      }
      ...
      window.alert('錯誤. 選擇圖片時發生錯誤: ' + error.message);
    }
  };
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      window.alert('錯誤' + error.message || '上傳失敗！');
  ...
  const handleSave = async () => {
    if (!productName.trim()) {
      window.alert('錯誤: 產品名稱不能為空');
      return;
    }
    const stockValue = parseInt(stock, 10);
    if (isNaN(stockValue) || stockValue < 0) {
      window.alert('錯誤: 庫存數量必須為非負整數');
      return;
    }
    ...
    try {
      ...
      if (error) throw error;
      const result = window.confirm('產品資訊已更新');
      if (result) {
        navigation.goBack();
      }
      ...
      window.alert('錯誤: 無法更新產品資訊，請稍後重試');
    } finally {
      setIsLoading(false);
    }
  };
