...
const CustomerInfo = ({ navigation, route }) => {
  ...
  const fetchCustomerDetails = async () => {
    ...
    try {
      ...
      window.alert('錯誤', '無法加載客戶詳情，請稍後重試');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {isLoading ? (
        ...
        <View style={{maxHeight:Dimensions.get("window").height-Dimensions.get("window").height*0.2}}>
          <ScrollView
