...
import { View, Text, Image, TouchableOpacity, Alert, StyleSheet, ScrollView, ActivityIndicator, SafeAreaView, TextInput, Modal, Dimensions } from 'react-native';
...
const HandleOrder = ({ route, navigation }) => {
  ...
  const handleApprovePayment = async () => {
    ...
    const isConfirmed = window.confirm('确定要批準此付款吗？');
  ...
  const handleRejectPayment = async () => {
    ...
    //           window.alert('成功: 付款已被拒絕，用户需重新上传证明');
    //         } catch (error) {
    //           console.error('Error rejecting payment:', error.message || error);
    //           window.alert('错误: 拒絕付款失败');
    //         }
    //       },
    //     },
    //   ]
    // );
    const isConfirmed = window.confirm('确定要拒絕此付款吗？付款证明将被删除，用户需要重新上传。');
  ...
  const handleShipOrder = async () => {
    ...
    //           window.alert('成功: 订单已发货，库存已扣除，客户已收到通知');
    //         } catch (error) {
    //           console.error('Error shipping order:', error.message || error);
    //           window.alert(`错误: 发货失败: ${error.message || '请稍后重试'}`);
    //         }
    //       },
    //     },
    //   ]
    ...
    const isConfirmed = window.confirm('确定要为此订单发货吗？此操作将扣除库存并通知客户。');
  ...
  const handleMarkAsDelivered = async () => {
    ...
    //           setShowLocationModal(false);
    //           window.alert('成功: 訂單已標記為已送達，客戶已收到通知');
    //         } catch (error) {
    //           console.error('Error marking as delivered:', error.message || error);
    //           window.alert('錯誤: 標記送達失敗');
    //         }
    //       },
    //     },
    //   ]
    // );
    const isConfirmed = window.confirm('確定要將此訂單標記為已送達嗎？');
  ...
  const isAtDeliveryAddress2 = (newLocation2) => {
    if (!order || !newLocation2) return false;
    const deliveryAddress = formatDeliveryAddress();
    return newLocation2.trim().toLowerCase() === deliveryAddress.toLowerCase();
  };
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { maxHeight: Dimensions.get("window").height * 0.85 }]}>
      ...
      <Modal visible={showLocationModal} animationType="slide" transparent>
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <View style={{ flex: 1, flexDirection: "row" }}><Text style={styles.modalTitle}>更新物流位置</Text>
            </View>
            <TextInput
              ...
              onChangeText={(text) => {
                setNewLocation(text);
                const b = isAtDeliveryAddress2(text);
            ...
            <View style={{ flex: 1, flexDirection: "row", alignItems:"center" }}>
              <Text style={{ fontSize: 16 }}>已送达</Text>
              <CheckBox
                checked={toggleCheckBox}
                onIconPress={() => {
                  if (toggleCheckBox) {
                    setNewLocation(TypingLocation);
                  }
                  else {
                    setTypingLocation(newLocation);
                    setNewLocation(formatDeliveryAddress());
                  }
      ...
      <View style={{
        flex: 1, position: "absolute", top: windowHeight - 126, left: 0,
      }}>
        <AdminBar navigation={navigation} activeScreen="SalesHome" />
      </View>
