...
const RegisterForm = ({ navigation }) => {
  ...
  const handleNext = () => {
    if (!email.trim() || !password.trim() || !username.trim() || !confirmPassword.trim()) {
      window.alert('錯誤請填寫所有必填字段');
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      window.alert('錯誤，請輸入有效的電子郵件地址');
      return;
    }
    if (!passwordRegex.test(password)) {
      window.alert('密碼必須包含至少8個字符，包括大寫字母、小寫字母、數字和特殊字符（!@#$%^&*）');
      return;
    }
    if (password !== confirmPassword) {
      window.alert('錯誤，密碼和確認密碼不匹配');
      return;
    }
    setStep(2);
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const result = window.confirm('註冊成功!');
  if (result) {
    ...
    navigation.replace('HomePage');
      ...
      window.confirm('錯誤'+ error.message || '註冊失敗，請稍後重試');
    } finally {
      setIsLoading(false);
    }
  };
