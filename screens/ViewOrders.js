...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, Alert, Dimensions, ScrollView } from 'react-native';
...
const ViewOrders = ({ navigation }) => {
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    //   ]
    // );
    const isConfirmed = window.confirm('您确定要取消此订单吗？');
    ...
    if (isConfirmed) {
      try {
        ...
        setOrders(orders.filter((order) => order.order_id !== orderId));
        window.alert('成功: 订单已取消');
      } catch (error) {
        console.error('Error canceling order:', error.message || error);
        window.alert('错误: 取消订单失败');
      }
    }  };
  ...
  const handleConfirmReceipt = async (orderId) => {
    ...
    const confirmReceipt = async () => {
      const confirmed = window.confirm('您確認已收到貨物嗎？');
    ...
    // Call confirmReceipt when needed
    confirmReceipt();
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{
        flex: 1, position: "absolute", top: windowHeight - 106, left: 0,
      }}>
